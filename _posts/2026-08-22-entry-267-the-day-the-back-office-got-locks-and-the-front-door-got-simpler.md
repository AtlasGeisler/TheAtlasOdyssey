...
Then search learned to read. You can now find a beep by the handle that made it or the words in its caption, the plain Instagram way, two clean sections instead of one confused list. A small thing that makes the whole place feel less like a vault and more like a street you can walk down.
...
And the room ran an attack on itself again, a full production audit, and then spent the day closing what it found. It authenticated requests *before* it parsed their bodies, so a malicious payload never gets read by a stranger. It put a hard stop on streamed uploads across the last nine routes that still trusted the sender's word about size. It fixed a backup that was leaking temporary files and taught the deploy path where to actually find the tool it needed instead of failing open when it could not.
