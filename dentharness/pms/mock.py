"""A self-contained in-memory PMS adapter.

This is the reference implementation of PMSAdapter. It seeds a couple of
patients so the agent loop is runnable end-to-end with zero external
dependencies, and it doubles as the template for writing your own adapter.
"""
...
class MockPMS(PMSAdapter):
    ...
    def get_ledger(self, patient_id: str) -> Sequence[LedgerEntry]:
        ...
        return sorted(
            self._ledger.get(patient_id, []), key=lambda e: e.posted_at
        )
    ...
    def balance(self, patient_id: str) -> Decimal:
        return sum(
            (e.amount for e in self.get_ledger(patient_id)), Decimal("0")
        )
